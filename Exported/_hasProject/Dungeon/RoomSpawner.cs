        ...
        */
        ...
        private void Spawn()
        {
            if (!spawned)
            {
                ...
                if (needsRoomWith == ROOM_DIRECTIONS.BOTTOM)
                {
                    ...
                    spawnedRoom = Instantiate(picked, transform.position, picked.transform.rotation, templates.transform);
                }
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            if (other.CompareTag("SpawnPoint"))
            {
                ...
                {
                    var cr = Instantiate(templates.closedRoom, transform.position, Quaternion.identity, templates.transform);

                    // Make sure we aren't overlapping any other rooms
                    if (Vectors.Dist(cr.transform.position, templates.startRoom.transform.position) <= 1f)
                    {
                        Destroy(cr);
                    }
